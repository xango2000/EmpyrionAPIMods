...
namespace AdminCommands
{
    public class AdminCommands : ModInterface
    {
        ...
        Dictionary<ushort, Tracker.Request.TrackData> SeqNrDict = new Dictionary<ushort, Request.TrackData> { };
        ...
        {
            if (SeqNrCounter == 35000)
            {
                SeqNrCounter = 1500;
            }
            else
            {
                SeqNrCounter = SeqNrCounter++;
            }
            return SeqNrCounter;
        }
        ...
        public void Game_Event(CmdId cmdId, ushort seqNr, object data)
        {
            try
            {
                switch (cmdId)
                {
                    case CmdId.Event_ChatMessage: //Automatic
                        break;
                    ...
                    case CmdId.Event_Player_Connected://Automatic
                        ...
                        SeqNrDict[NewSeqNr] = Request.SeqNrTracker(CmdId.Request_Player_Info, NewSeqNr, PlayerConnected.id);
                        break;
                    ...
                    try
                    {
                        switch (cmdId)
                        {
                            ...
                            case CmdId.Event_Player_Info://Triggered
                                ...
                                if (SeqNrDict[seqNr].empyrionID == PlayerInfoReceived.entityId)
                                {
